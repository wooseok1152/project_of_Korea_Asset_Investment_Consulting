...
from make_exhaustion_rate_df import make_exhaustion_rate_df
...
logger = logger.get_logger("SEC11T")
...
try :
    ...
    df_exhaustion_rate_by_base_date_and_is = pd.concat([make_exhaustion_rate_df(i, date) for i, date in enumerate(date_range(start_date, end_date))])
    logger.info("making df_exhaustion_rate_by_base_date_and_is finished")

    result_log = df_to_csv(df_exhaustion_rate_by_base_date_and_is, csv_material_file_path)
    ...
    logger.info("batch for inserting data into SEC11T finished")
    logger.info(" ")
