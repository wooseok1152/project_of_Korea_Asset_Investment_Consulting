...
from make_OHLCV_df import make_OHLCV_df
...
logger = logger.get_logger("SEC02T")
...
try :
    ...
    df_OHLCV_by_base_date_and_is = pd.concat([make_OHLCV_df(i, date) for i, date in enumerate(date_range(start_date, end_date))])
    ...
    logger.info("batch for inserting data into SEC02T finished")
    logger.info(" ")
