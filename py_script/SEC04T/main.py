...
from make_market_cap_df import make_market_cap_df
...
logger = logger.get_logger("SEC04T")
...
try :
    ...
    df_market_cap_by_base_date_and_is = pd.concat([make_market_cap_df(i, date) for i, date in enumerate(date_range(start_date, end_date))])
    logger.info("making df_market_cap_by_base_date_and_is finished")

    result_log = df_to_csv(df_market_cap_by_base_date_and_is, csv_material_file_path)
    ...
    logger.info("batch for inserting data into SEC04T finished")
    logger.info(" ")
