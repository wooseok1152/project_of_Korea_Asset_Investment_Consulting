...
from make_etf_OHLCV_df import make_etf_OHLCV_df
...
logger = logger.get_logger("SEC51T")
...
try :
    ...
    df_etf_OHLCV_by_base_date_and_is = pd.concat([make_etf_OHLCV_df(i, date) for i, date in enumerate(date_range(start_date, end_date))])
    logger.info("making df_etf_OHLCV_by_base_date_and_is finished")

    result_log = df_to_csv(df_etf_OHLCV_by_base_date_and_is, csv_material_file_path)
    ...
    logger.info("batch for inserting data into SEC51T finished")
    logger.info(" ")
